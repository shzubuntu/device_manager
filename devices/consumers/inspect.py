...
logger = logging.getLogger('devices.inspect')
...
class InspectionConsumer(AsyncWebsocketConsumer):
    ...
    async def report_init(self,device_ids,command_ids,server_commands,network_commands):
        ...
        # 初始化巡检目录
        self.inspection_dir = os.path.join(settings.DIR_INFO['REPORT_DIR'],'inspect',report_id)
    ...
    async def receive(self, text_data):
        try:
            ...
            if data['type'] == 'inspect.start':
                ...
                logger.info("收到再次巡检命令")
                # 在历史巡检记录页面点击再次执行按钮的操作
                his_inspection_id = data['id']
                # 在这里执行巡检逻辑
                history_dir = os.path.join(settings.DIR_INFO['REPORT_DIR'], 'inspect',his_inspection_id)
            ...
            if cache_devices:
                for item in cache_devices:
                    if item.id == device_id:
                        logger.debug(f"设备命中缓存{item}")
                        device = item
            else:
                device = Device.objects.get(id=device_id)
                logger.debug(f"设备未命中缓存{device}")
            return {
