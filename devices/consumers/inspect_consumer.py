...
import logging
logger = logging.getLogger('devices.inspection_consumer')
...
class InspectionConsumer(AsyncWebsocketConsumer):
    def __init__(self, *args, **kwargs):
        ...
        # 新增报告存储结构
        self.reports  = {}  # {inspection_id: {metadata, results}}
        self.inspection_dir = ''
    ...
    async def report_init(self,device_ids,command_ids,server_commands,network_commands):
        ...
        # 初始化巡检目录
        self.inspection_dir = os.path.join(settings.BASE_DIR,'results',report_id)
        if not os.path.exists( self.inspection_dir):
            os.makedirs( self.inspection_dir)
        # 初始化巡检报告信息
        self.reports[report_id]  = {
            'report_dir': self.inspection_dir, # 报告目录
        ...
        logger.info(f'巡检报告初始化成功，ID: {report_id}, 信息: {self.reports[report_id]}')
        # 巡检任务进度跟踪
        self.progress_tracker = {
    ...
    async def receive(self, text_data):
        try:
            ...
            if data['type'] == 'inspect.start':
                ...
                #}))
                logger.info(f'收到巡检请求，设备ID: {device_ids}, 命令ID: {command_ids}')

                await self.handle_inspect(device_ids, command_ids,server_commands,network_commands)
            elif data['type'] == 'inspect.again':
                # 在历史巡检记录页面点击再次执行按钮的操作
                his_inspection_id = data['id']
                # 在这里执行巡检逻辑
                history_dir = os.path.join(settings.BASE_DIR, 'results',his_inspection_id)
    ...
    async def handle_inspect(self, device_ids, command_ids,server_commands,network_commands):
        #处理空字符串
        device_ids = [device_id for device_id in device_ids if device_id] # 删除空字符串
        command_ids = [cmd for cmd in command_ids if cmd] # 删除空字符串
        server_commands = [cmd for cmd in server_commands if cmd] # 删除空字符串
        network_commands = [cmd for cmd in network_commands if cmd] # 删除空字符串
        #初始化巡检报告信息
        await self.report_init(device_ids,command_ids,server_commands,network_commands)
        # 执行巡检
        logger.info(f'开始执行巡检任务，设备ID: {device_ids}, 命令ID: {command_ids}, server_commands: {server_commands}, network_commands: {network_commands}')
        await self.execute_commands(device_ids, command_ids,server_commands,network_commands)
        logger.info(f'巡检任务执行完成，报告ID: {self.current_report_id}')
        # 发送完成消息
        await self.send(json.dumps({ 
            'type': 'report.created', 
            'report_id': self.current_report_id
    ...
    async def execute_commands(self, device_ids, command_ids,server_commands,network_commands):
        try:
            ...
            # 生成巡检记录文件
            with open(os.path.join(self.inspection_dir,"index.json"),'w') as f:
    ...
    def _get_command_credentials_sync(self, command_ids):
        ...
        try:
            ...
            logger.error(f"命令ID {command_ids} 不存在")
            ...
            if cache_devices:
                for item in cache_devices:
                    if item.id == device_id:
                        logger.debug(f"设备{device_id}命中缓存")
                        device = item
            else:
                device = Device.objects.get(id=device_id)
                logger.debug(f"设备{device}未命中缓存")
            return {
    ...
    def _handle_network_device_sync(self, device_info, commands):
        ...
        try:
            conn = ConnectHandler(
                ...
                timeout=20,
                session_log=os.path.join(self.inspection_dir,f"{device_info['name']}__{device_info['ip']}.log")
            )
            logger.info(f"连接到网络设备({device_info['name']})({device_info['ip']}) 成功, 开始执行命令({commands})")
            # 串行执行命令
            for cmd in commands:
            ...
            self.send_instant_progress_update()
            if conn:
                logger.info(f"关闭网络设备({device_info['name']})({device_info['ip']})连接")
                conn.disconnect()
    ...
    def _handle_generic_device_sync(self, device_info, commands):
        ...
        try:
            ...
            logger.info(f"连接到通用SSH设备({device_info['name']})({device_info['ip']}) 成功, 开始执行命令({commands})")
            # 串行执行命令
            for cmd in commands:
                logger.debug(f"{device_info['name']}({device_info['ip']})执行命令{cmd}")
                try:
            ...
            if ssh:
                logger.info(f"关闭通用SSH设备({device_info['name']})({device_info['ip']})连接")
                ssh.close()
    ...
    async def send_completion_message(self):
        ...
        if report_id in self.reports: 
            ...
            """
            logger.info(f"已生成巡检报告({report_id})")
            # 直接发送给当前连接
